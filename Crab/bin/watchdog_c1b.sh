#!/usr/bin/env bash
# C1b (LoRA expanded q,k,v,o, encoder_lr 5e-5) watchdog.
# Same structure as bin/watchdog_c14.sh — matches the C1b process and writes
# to C1b-specific status / log / marker files.
#
# Start:  nohup bash bin/watchdog_c1b.sh > /dev/null 2>&1 & disown
# Stop:   pkill -f watchdog_c1b.sh
#
# Morning check:
#   cat experiments/strategyA_c1b_watchdog_status.txt
#   tail -50 experiments/strategyA_c1b_watchdog.log
#   ls experiments/C1b_*.marker 2>/dev/null

set -u

CRAB=/home/brant/Project/SAILER_test/Crab
LOG_DIR=$CRAB/experiments
WATCH_LOG=$LOG_DIR/strategyA_c1b_watchdog.log
STATUS=$LOG_DIR/strategyA_c1b_watchdog_status.txt
TRAIN_LOG=$(ls -t $LOG_DIR/strategyA_c1b_launch_*.log 2>/dev/null | head -1)
...
write_status() {
  local state="$1"; shift
  cat > "$STATUS" <<EOF
=== C1b (LoRA expanded, encoder_lr 5e-5) watchdog status ===
...
while true; do
  CYCLE=$((CYCLE + 1))
  PID=$(pgrep -of "train_crab_lora.py.*encoder_lr 5e-5" 2>/dev/null)
  ...
  if [ -z "${PID:-}" ]; then
    if [ -n "$TRAIN_LOG" ] && grep -aq "INFO - Done\." "$TRAIN_LOG"; then
      ...
      touch "$LOG_DIR/C1b_COMPLETE_$(date +%Y%m%d_%H%M%S).marker"
      break
    fi
    if [ -n "$TRAIN_LOG" ] && grep -aiqE "out of memory|cuda error|runtimeerror|traceback" "$TRAIN_LOG"; then
      ...
      touch "$LOG_DIR/C1b_CRASHED_$(date +%Y%m%d_%H%M%S).marker"
      break
    fi
    log "PROCESS GONE — no error marker"
    write_status disappeared "no PID, no error. inspect $TRAIN_LOG"
    touch "$LOG_DIR/C1b_PROCESS_GONE_$(date +%Y%m%d_%H%M%S).marker"
    break
  fi
